<?php
    ...
    switch ($Action):
        case 'search':
            ...
            $Read->FullRead("SELECT pdt_title, pdt_name, pdt_cover, pdt_price, pdt_offer_price, pdt_offer_start, pdt_offer_end FROM " . DB_PDT . " WHERE (pdt_title LIKE '%' :search '%' OR pdt_tags LIKE '%' :search '%' OR pdt_subtitle LIKE '%' :search '%') AND pdt_status = :status AND pdt_inventory >= :status", "search={$search}&status=1");
            ...
            if ($Read->getResult()):
                ...
                foreach ($Read->getResult() as $PDT):
                    if ($PDT['pdt_offer_price'] && $PDT['pdt_offer_start'] <= date('Y-m-d H:i:s') && $PDT['pdt_offer_end'] >= date('Y-m-d H:i:s')):
                        ...
                        $jSON['search'] .= "<li><a href='" . BASE . "/produto/{$PDT['pdt_name']}#pdt' title='{$PDT['pdt_title']}'><img class='on-contrast-force-gray' src='" . BASE . "/uploads/{$PDT['pdt_cover']}' alt='{$PDT['pdt_title']}' title='{$PDT['pdt_title']}'/><p>{$PDT['pdt_title']}<br/><span class='old_price'>R$ " . number_format($PDT['pdt_price'], 2, ',', '.') . "</span> <b class='new_price'>R$ " . number_format($PDT['pdt_offer_price'], 2, ',', '.') . "</b></p></a></li>";
                    else:
                        $jSON['search'] .= "<li><a href='" . BASE . "/produto/{$PDT['pdt_name']}#pdt' title='{$PDT['pdt_title']}'><img src='" . BASE . "/uploads/{$PDT['pdt_cover']}' alt='{$PDT['pdt_title']}' title='{$PDT['pdt_title']}'/><p>{$PDT['pdt_title']}<br/><b class='new_price'>R$ " . number_format($PDT['pdt_price'], 2, ',', '.') . "</b></p></a></li>";
                    endif;
                endforeach;
                $jSON['search'] .= '</ul>';
            endif;
        ...
        case 'get_pdt':
            ...
            if ($Read->getResult()):
                ...
                $Read->FullRead("SELECT SUM(ranking) as total FROM " . DB_COMMENTS . " WHERE pdt_id = :pid{$CommentModerate}", "pid={$pdt_id}");
                $TotalAval = $Read->getResult()[0]['total'];
                $TotalRanking = $Aval * 5;
                $getRanking = ($TotalAval ? (($TotalAval / $TotalRanking) * 50) / 10 : 0);
                $Ranking = str_repeat("<li class='icon-star-full'></li>", intval($getRanking)) . str_repeat("<li class='icon-star-empty'></li>", 5 - intval($getRanking));
                ...
                $jSON['pdt'] .= "<img class='j_focus_image image-zoom on-contrast-force-gray' src='" . BASE . "/uploads/{$pdt_cover}' alt='{$pdt_title}' title='{$pdt_title}' data-zoom='" . BASE . "/uploads/{$pdt_cover}'/>";
                $jSON['pdt'] .= "</div>";
                $jSON['pdt'] .= "<div class='product_image_gallery'>";
                $jSON['pdt'] .= "<img class='j_select_gallery active on-contrast-force-gray' src='" . BASE . "/uploads/{$pdt_cover}' alt='{$pdt_title}' title='{$pdt_title}'/>";

                $Read->ExeRead(DB_PDT_GALLERY, "WHERE product_id = :id", "id={$pdt_id}");
                if ($Read->getResult()):
                    foreach ($Read->getResult() as $GALLERY):
                        $jSON['pdt'] .= "<img class='j_select_gallery on-contrast-force-gray' src='" . BASE . "/uploads/{$GALLERY['image']}' alt='{$pdt_title}' title='{$pdt_title}'/>";
                    endforeach;
                endif;
                ...
                if (!empty($_SESSION['userLogin']['user_id'])):
                    $Read->FullRead(
                        "SELECT wishlist_id FROM " . DB_PDT_WISHLIST . " WHERE pdt_id = :pdt AND user_id = :user",
                        "pdt={$pdt_id}&user={$_SESSION['userLogin']['user_id']}"
                    );

                    $classWishlist = (!empty($Read->getResult()) ? 'j_toggle_wishlist active' : 'j_toggle_wishlist');
                    $iconWishlist = (!empty($Read->getResult()) ? 'icon-heart animate__infinite animate__heartBeat animate__animated active' : 'icon-heart');
                ...
                $jSON['pdt'] .= "</span>";
				$jSON['pdt'] .= ((!empty($pdt_offer_price) && $pdt_offer_start <= date('Y-m-d H:i:s') &&
					$pdt_offer_end >= date('Y-m-d H:i:s')) ? "<span class='pdt_discount'> {$pdt_offer_percent}% OFF</span>" : '');
                $jSON['pdt'] .= "</div>";
                ...
                $jSON['pdt'] .= "{$Ranking}";
                $jSON['pdt'] .= "</ul>";
				$jSON['pdt'] .= "<p style='margin-bottom: 20px; color: #5e5e5e'>{$pdt_subtitle}</p>";
                ...
                if ($pdt_inventory >= 1):
                    ...
                    if ($Read->getResult() || !empty($pdt_parent)):
                        ...
                        if ($Read->getResult()):
                            ...
                            foreach ($Read->getResult() as $StockVar):
                                ...
                                $jSON['pdt'] .= "<i class='icon-radio-checked'></i>";
                                $jSON['pdt'] .= "</div>";
                                $jSON['pdt'] .= "</a> ";
                            endforeach;
                        ...
                        if ($Read->getResult()):
                            ...
                            $jSON['pdt'] .= "</div>";
                            $jSON['pdt'] .= "<img class='image_loading on-contrast-force-gray' src='" . INCLUDE_PATH . "/images/loading.gif' alt='Carregando...' title='Carregando...'/>";
                            $jSON['pdt'] .= "</div>";
                        else:
                            $openByColor = false;
                        endif;
                    ...
                    if ($Read->getResult()):
                        ...
                        foreach ($Read->getResult() as $StockVar):
                            ...
                            $jSON['pdt'] .= "<span><i class='icon-checkmark'></i></span>";
                            $jSON['pdt'] .= "</label> ";
                        endforeach;

                        $jSON['pdt'] .= "</div>";
                        $jSON['pdt'] .= "</div>";
                        $jSON['pdt'] .= "<img class='image_loading on-contrast-force-gray" . ($openByColor || $openByPrint ? ' invisible' : '') . "' src='" . INCLUDE_PATH . "/images/loading.gif' alt='Carregando...' title='Carregando...'/>";
                        $jSON['pdt'] .= "</div>";
                    else:
                        $openBySizes = false;
                    endif;
                    ...
                    $jSON['pdt'] .= "<div>";
	                $jSON['pdt'] .= "<button class='btn_purchase' style='margin-bottom: 12px; cursor: pointer'><i class='fas fa-shopping-bag bell font_white'></i> " .
		                ECOMMERCE_BUTTON_TAG .
		                "</button>";

	                /*AQUI*/
	                if (
		                $pdt_offer_price <= 6000
		                && $pdt_offer_start <= date('Y-m-d H:i:s')
		                && $pdt_offer_end >= date('Y-m-d H:i:s')

	                ) {
		                $Read->FullRead("SELECT tg_title, tg_trigger FROM " . DB_TRIGGERS . " WHERE tg_status = :st AND tg_type = :type AND tg_discount >= :discount ORDER BY RAND() LIMIT :limit", "st=1&type=1&discount={$pdt_offer_percent}&limit=1");

		                if ($Read->getResult()) {
			                extract($Read->getResult()[0]);
			                $jSON['pdt'] .= "<p class='urgent_mental_trigger' style='display: none'><i class='bell fa fa-bell'></i> {$tg_title} <br/><small>" . str_replace('{{discount}}',
					                $pdt_offer_percent, $tg_trigger) . "</small></p><br>";
		                }

	                } elseif (
		                $pdt_offer_price >= 6000
		                && $pdt_offer_price < 8000
		                && $pdt_offer_start <= date('Y-m-d H:i:s')
		                && $pdt_offer_end >= date('Y-m-d H:i:s')
	                ) {
		                $Read->FullRead("SELECT tg_title, tg_trigger FROM " . DB_TRIGGERS . " WHERE tg_status = :st AND tg_type = :type AND tg_min_value <= :min_value ORDER BY RAND() LIMIT :limit",
			                "st=1&type=3&min_value={$pdt_offer_price}&limit=1");

		                if ($Read->getResult()) {
			                extract($Read->getResult()[0]);
			                $jSON['pdt'] .= "<p class='urgent_mental_trigger' style='display: none'><i class='bell fa fa-bell'></i> {$tg_title} <br/><small>{$tg_trigger}</small></p><br>";
		                }
		                ...
		                $Read->FullRead("SELECT tg_title, tg_trigger FROM " . DB_TRIGGERS . " WHERE tg_status = :st AND tg_type = :type AND tg_min_value <= :min_value ORDER BY RAND() LIMIT :limit",
			                "st=1&type=2&min_value={$pdt_offer_price}&limit=1");
		                ...
		                $Read->FullRead("SELECT tg_title, tg_trigger FROM " . DB_TRIGGERS . " WHERE tg_status = :st AND tg_type = :type ORDER BY RAND() LIMIT :limit", "st=1&type=5&limit=1");
                   ...
                   // $jSON['pdt'] .= "<p>Troca garantida. <a class='wc_tab' href='#warranty' title='Veja as regras'>Veja as regras.</a> <span>Válido em todo território nacional</span></p>";
                    ...
                    $jSON['pdt'] .= "<img class='image_loading on-contrast-force-gray" . ($openByColor || $openByPrint || $openBySizes ? ' invisible' : '') . "' src='" . INCLUDE_PATH . "/images/loading.gif' alt='Carregando...' title='Carregando...'/>";
                    $jSON['pdt'] .= "</div>";
                    $jSON['pdt'] .= "</form>";
                endif;
                ...
                if (!empty($pdt_content)):
                   ...
                   // $jSON['pdt'] .= "<li><a class='wc_tab' href='#warranty' title='Especificações Técnicas'>Especificações Técnicas</a></li>";
                    $jSON['pdt'] .= "</ul>";
        ...
        case 'get_sizes':
            ...
            if ($Read->getResult()):
                $jSON['sizes'] = null;
                foreach ($Read->getResult() as $StockVar):
                    $jSON['sizes'] .= "<label class='combo_select_size' title='{$StockVar['attr_size_title']}'>{$StockVar['attr_size_code']} <input type='radio' name='stock_id' value='{$StockVar['stock_id']}'/><span><i class='icon-checkmark'></i></span></label> ";
                endforeach;
            endif;
            break;
                ...
                if ($Read->getResult()):
                    ...
                    if ($Read->getResult()):
                        foreach ($Read->getResult() as $PDT):
                            ...
                            $jSON['content'] .= "<img class='on-contrast-force-gray' src='" . BASE . "/uploads/{$pdt_cover}' alt='{$pdt_title}' title='{$pdt_title}'/>";
                            $jSON['content'] .= "</a>";

                            if ($discount):
                                $jSON['content'] .= "<div class='discount_product'>";
                                $jSON['content'] .= "<p><a href='" . BASE . "/produto/{$pdt_name}' title='{$pdt_title}'>-{$discount}% <i class='icon-radio-unchecked faa-burst animated'></i></a></p>";
                                $jSON['content'] .= "</div>";
                            endif;

                            $jSON['content'] .= "<div class='action_purchase'>";
                            $jSON['content'] .= "<a href='" . BASE . "/produto/{$pdt_name}#pdt' title='Comprar'>";
                            $jSON['content'] .= "<i class='icon-cart'></i>";
                            $jSON['content'] .= "</a>";
                            $jSON['content'] .= "</div>";

                            $jSON['content'] .= "<div class='action_view'>";
                            $jSON['content'] .= "<a class='j_view_product' href='#' title='Espiar' data-pdt-id='{$pdt_id}'>";
                            $jSON['content'] .= "<i class='icon-eye-plus'></i>";
                            $jSON['content'] .= "</a>";
                            $jSON['content'] .= "</div>";
                            ...
                            if (!empty($_SESSION['userLogin']['user_id'])):
                                ...
                                $classWishlist = (!empty($Read->getResult()) ? 'j_toggle_wishlist active' : 'j_toggle_wishlist');
                                $iconWishlist = (!empty($Read->getResult()) ? 'fas fa-heart animate__infinite animate__pulse animate__animated' : 'far fa-heart');
                                $titleWishlist = (!empty($Read->getResult()) ? 'Remover dos Favoritos' : 'Adicionar aos Favoritos');
                                $attrUserId = " data-user-id='{$_SESSION['userLogin']['user_id']}'";
                            else:
                                $classWishlist = 'j_force_login';
                            ...
                            if ($Read->getResult() || !empty($pdt_parent)):
                                ...
                                if ($Read->getResult()):
                                    ...
                                    foreach ($Read->getResult() as $StockVar):
                                        ...
                                        $jSON['content'] .= "<i class='icon-radio-checked'></i>";
                                        $jSON['content'] .= "</div>";
                                        $jSON['content'] .= "</a> ";
                                    endforeach;
                                    $jSON['content'] .= "</div>";
                                ...
                                if ($Read->getResult()):
                                    ...
                                    $jSON['content'] .= "<img class='image_loading on-contrast-force-gray' src='" . INCLUDE_PATH . "/images/loading.gif' alt='Carregando...' title='Carregando...'/>";
                                    $jSON['content'] .= "</div>";
                                else:
                                    $openByColor = false;
                                endif;
                            ...
                            if ($Read->getResult()):
                                ...
                                foreach ($Read->getResult() as $StockVar):
                                    ...
                                    $jSON['content'] .= "<span><i class='icon-checkmark'></i></span>";
                                    $jSON['content'] .= "</label> ";
                                endforeach;
                                $jSON['content'] .= "</div>";
                                $jSON['content'] .= "</div>";

                                $jSON['content'] .= "<img class='image_loading on-contrast-force-gray" . ($openByColor || $openByPrint ? ' invisible' : '') . "' src='" . INCLUDE_PATH . "/images/loading.gif' alt='Carregando...' title='Carregando...'/>";
                                $jSON['content'] .= "</div>";
                            endif;
                            $jSON['content'] .= "</form>";
                    ...
                    foreach ($comboResult as $PDT):
                        ...
                        $jSON['content'] .= "<span class='j_combo_hide icon-cross' data-pdt-id='{$pdt_id}'></span>";
                        $jSON['content'] .= "</div>";
                        ...
                        if (!empty($_SESSION['userLogin']['user_id'])):
                            ...
                            $classWishlist = (!empty($Read->getResult()) ? 'j_toggle_wishlist active' : 'j_toggle_wishlist');
                            $iconWishlist = (!empty($Read->getResult()) ? 'icon-heart animate__infinite animate__pulse animate__animated' : 'icon-heart');
                            $titleWishlist = (!empty($Read->getResult()) ? 'Remover dos Favoritos' : 'Adicionar aos Favoritos');
                            $attrUserId = " data-user-id='{$_SESSION['userLogin']['user_id']}'";
                        else:
                            $classWishlist = 'j_force_login';
                            $iconWishlist = 'icon-heart';
                            $titleWishlist = 'Adicionar aos Favoritos';
                            $attrUserId = '';
                        endif;
