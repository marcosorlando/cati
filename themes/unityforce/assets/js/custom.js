const PATH = $("link[rel='paththeme']").attr("href");
...
document.addEventListener("DOMContentLoaded", function () {
    // Código para fixar o menu
    const menu = document.getElementById("menu"); // ou use document.querySelector(".menu") para classes
    const logo = document.getElementById("logo-header")
    // Armazena a posição original do menu
    const sticky = menu.offsetTop;
    
    // Adiciona um ouvinte de evento para rolar a página
    window.onscroll = function () {
        // Verifica a posição de rolagem e adiciona a classe "sticky" se necessário e muda versão do logo
        if (window.pageYOffset > sticky) {
            menu.classList.add("sticky");
            logo.src = PATH + "/assets/images/header-logo-alt.png";
            
            $('a.nav-link').css("color","#ffd002")
        } else {
