...
<div class="dashboard_content">
		...
		<div class="panel wc_onlinenow">
			<?php
				...
				$Read->ExeRead(DB_SEARCH,
					" WHERE 1 = 1 {$Where} AND search_publish IS NULL AND search_origin = :pdt ORDER BY search_commit DESC, search_count DESC LIMIT :limit OFFSET :offset",
					"pdt=PROD&limit={$Pager->getLimit()}&offset={$Pager->getOffset()}&{$ParseString}");
				if (!$Read->getResult()):
					$Pager->ReturnPage();
					echo Erro('<span>Seus usuários ainda não pesquisaram por produtos no site. Assim que isso acontecer você poderá receber dicas de conteúdo pelas pesquisas realizadas!</span>',
						E_USER_NOTICE);
				else:
	...
	<?php
		$Pager->ExePaginator(DB_SEARCH,
			" WHERE 1 = 1 {$Where} AND search_publish IS NULL AND search_origin = :pdt ",
