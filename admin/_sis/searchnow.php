...
<div class="dashboard_content">
		...
		<div class="panel wc_onlinenow">
			<?php
				...
				$Read->ExeRead(DB_SEARCH,
					" WHERE 1 = 1 {$Where} AND search_publish IS NULL AND search_origin = :post ORDER BY search_commit DESC, search_count DESC LIMIT :limit OFFSET :offset",
					"post=POST&limit={$Pager->getLimit()}&offset={$Pager->getOffset()}&{$ParseString}");
				...
				if (!$Read->getResult()):
					...
					foreach ($Read->getResult() as $Search):
						...
						echo "
                        <article>
                           <h1 class='icon-search'><a href='dashboard.php?wc=posts/search&s=" . urlencode($search_key) . "' title='Ver resultados'>" . ($search_parse == 'month' ?
	...
	<?php
		$Pager->ExePaginator(DB_SEARCH,
					" WHERE 1 = 1 {$Where} AND search_publish IS NULL AND search_origin = :post ",
