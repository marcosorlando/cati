		...
		<div class="box box30">
			...
			<div class="post_create_categories">
				...
				<?php
					...
					if (!$Read->getResult()):
						echo "<br><br>";
						echo Erro('<span class="al_center icon-price-tags">Não existe categorias cadastradas!</span>',
							E_USER_WARNING);
					else:
						foreach ($Read->getResult() as $Categories):
							$Read->FullRead("SELECT category_id, category_title FROM " . DB_CATEGORIES . " WHERE category_parent = :parent",
			...
			<div class="panel">
					...
					if (APP_POSTS_INSTANT_ARTICLE):
						...
						<label class="label">
							...
							<select name="post_instant_article" required>
								<option value="0" <?= ($post_instant_article != '0' ? "selected='selected'" : ''); ?>>
									Não
								</option>
								<option value="1" <?= ($post_instant_article == '1' ? "selected='selected'" : ''); ?>>
									Sim
								</option>
							</select>
						</label>
				...
				<label class='label'>
					...
					<input style='font-size: 1em;' min="1" step="1" type='number' name='post_time' value="<?=
						$post_time; ?>"
					       required/>
				</label>
				...
				<label class="label">
					...
					<input type="text" class="jwc_datepicker" data-timepicker="true" readonly="readonly"
					       name="post_date"
					       value="<?= $post_date ? date('d/m/Y H:i', strtotime($post_date)) : date('d/m/Y H:i'); ?>"
					       required/>
				</label>
