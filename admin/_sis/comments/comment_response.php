    ...
    <section class="box box100">
        <div class="panel" style="padding: 30px;">
            <?php
                ...
                //USER
                $Read->LinkResult(DB_USERS, "user_id", "{$user_id}",
                ...
                //LIKES
                $Read->FullRead("SELECT user_id, user_name, user_lastname FROM " . DB_USERS . " WHERE user_id IN(SELECT user_id FROM " . DB_COMMENTS_LIKES . " WHERE comm_id = :comm)",
                    "comm={$Comm['id']}");
                if ($Read->getResult()):
                    $getLikes = [];
            ...
            <article class="ead_support_response" id="<?= $id; ?>" style="margin-bottom: 20px;">
                <div class="ead_support_response_avatar">
                    <img class="rounded"
                         src="<?= BASE; ?>/tim.php?src=<?= $user_task['user_thumb']; ?>&w=<?= round(AVATAR_W / 2); ?>&h=<?= round(AVATAR_H / 2); ?>"
                         alt="<?= $user_task['user_name']; ?>" title="<?= $user_task['user_lastname']; ?>"/>
                </div>
                <div class="ead_support_response_content">
                    <header class="ead_support_response_content_header">
                        <h1>Comentário de <a target="_blank"
                            <h1>Comentário de <a target="_blank"
                                                 href="dashboard.php?wc=users/create&id=<?= $user_task['user_id']; ?>"
                                                 title="<?= "{$user_task['user_name']} {$user_task['user_lastname']}"; ?>"><?= "{$user_task['user_name']} {$user_task['user_lastname']}"; ?></a>dia <?= date('d/m/Y H\hi',
                                    strtotime($created)); ?> <span class="j_comment_status"><?= $TicketStatus; ?></span>
                            </h1>
                            <p>Em: <a target="_blank" title="Ver <?= $Title; ?>"
                                      href="<?= BASE; ?>/<?= $Link; ?>#<?= $id; ?>"><?= $Title; ?></a></p>
                    </header>
                        ...
                        <?php
                            if ($status == 2):
                                ...
                                <span class="btn btn_green icon-checkmark j_comment_action ead_support_finish"
                                      data-action='comment_completed' id="<?= $id; ?>">Concluir</span>
                            <?php
                            endif;

                            //ACTIONS COMMENTS
                            $Read->FullRead("SELECT id FROM " . DB_COMMENTS_LIKES . " WHERE user_id = :user AND comm_id = :comm",
                                "user={$_SESSION['userLogin']['user_id']}&comm={$Comm['id']}");
                            if (!$Read->getResult()):
                                ?>
                                <a style="color: #fff; font-weight: bold;" href='#<?= $Comm['id']; ?>'
                                   class='btn btn_blue wc_comment_action icon-heart' id="<?= $Comm['id']; ?>"
                                   rel='<?= $Comm['id']; ?>' action='like' href='Gostei do Comentário'
                        ...
                        <?php
                            ...
                            if ($Reply):
                                foreach ($Reply as $ResponseReply):
                                    ...
                                    //VAR USER
                                    $Read->LinkResult(DB_USERS, "user_id", "{$ResponseReply['user_id']}",
                                        'user_id, user_name, user_lastname, user_email, user_thumb');
                                    ...
                                    echo "<article class='ead_support_response ead_support_response_reply reply' id='{$ResponseReply['id']}'>
                                                <div class='ead_support_response_avatar'>
                                                    ...
                                                    <header class='ead_support_response_content_header'>
                                                        <h1>Resposta de <a target='_blank' href='dashboard.php?wc=users/create&id= {$user_reply['user_id']}' title='{$user_reply['user_name']} {$user_reply['user_lastname']}'>{$user_reply['user_name']} {$user_reply['user_lastname']}</a> dia " . date('d/m/Y H\hi',
                                            strtotime($ResponseReply['created'])) . "</h1>
                                                    </header>
                ...
                <div class="ead_support_response_edit_modal response">
                    <form name="class_edit" action="" method="post" enctype="multipart/form-data">
                        <p class="title icon-pencil2">Atualizar Pergunta
                            de <?= "{$user_task['user_name']} {$user_task['user_lastname']}"; ?></p>
                        ...
                        <label class="label">
                            <textarea class="work_mce_basic" style="font-size: 1em;" name="comment"
                                      rows="3"><?= htmlspecialchars($comment); ?></textarea>
                        </label>
                ...
                <div class="ead_support_response_edit_modal remove">
                    <form name="class_add" action="" method="post" enctype="multipart/form-data">
                        <p class="title icon-warning">Enviar notificação
                            a <?= "{$user_task['user_name']} {$user_task['user_lastname']}"; ?>: (Opcional)</p>
                        <span class="btn btn_red icon-cross icon-notext ead_support_response_edit_modal_close j_ead_support_action_close"></span>
