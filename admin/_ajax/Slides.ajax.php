	...
	if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack):
		...
		switch ($Case):
			...
			case 'manager':
				...
				$PostData['slide_link'] = (!empty($PostData['slide_link']) ? $PostData['slide_link'] : '#content');
				$PostData['slide_title'] = (!empty($PostData['slide_title']) ? $PostData['slide_title'] : 'Banner Nº '
					. $SlideId);
				...
				$Read->FullRead("SELECT slide_image_mobile, slide_image_tablet, slide_image_desktop FROM " . DB_SLIDES . " WHERE slide_id = :id",
					"id={$SlideId}");

				if (empty($imageDesktop) && (!$Read->getResult() || !$Read->getResult()[0]['slide_image_desktop'])):
					$jSON['trigger'] = AjaxErro('<b class="icon-warning">ERRO AO CADASTRAR:</b> Favor envie uma imagem de destaque para <b>DESKTOP</b> nas medidas de ' . SLIDE_W . 'x' . SLIDE_H . 'px!',
						E_USER_ERROR);
				elseif (in_array('', $PostData)):
					$jSON['trigger'] = AjaxErro('<b class="icon-warning">ERRO AO CADASTRAR:</b> Para atualizar o destaque, favor preencha todos os campos!',
						E_USER_ERROR);
					$jSON['error'] = true;
				else:
					...
					$PostData['show_headline'] = (!empty($PostData['show_headline']) ? $PostData['show_headline'] : '0');
					...
					if (!empty($imageMobile)):
						...
						$Upload->Image($imageMobile, Check::Name($PostData['slide_title']) . '-' . 'mobile', SLIDE_W,
							'slides');
					...
					if (!empty($imageTablet)):
						...
						$Upload = new Upload('../../uploads/');
						$Upload->Image($imageTablet, Check::Name($PostData['slide_title']) . '-' . 'tablet', SLIDE_W,
							'slides');
					...
					if (!empty($imageDesktop)):
						...
						$Upload = new Upload('../../uploads/');
						$Upload->Image($imageDesktop, Check::Name($PostData['slide_title']) . '-' . 'desktop', SLIDE_W,
							'slides');
