		...
		switch ($Case):
			...
			case 'create':
				if (!empty($PostData['api_key']) && mb_strlen($PostData['api_key']) > 8):
					...
					$jSON['trigger'] = AjaxErro("<b class='icon-warning'>ERRO AO CRIAR APP:</b> Desculpe, mas não é seguro criar uma key com menos de 8 caracteres!",
						E_USER_WARNING);
				endif;
				break;
			...
			case 'license':
				...
				if (in_array('', $PostData)) {
					$jSON['trigger'] = AjaxErro("<b class='icon-warning'>LICENSE KEY:</b> Para licenciar um domínio é preciso informar seus dados e a chave da licença!",
						E_USER_WARNING);
				} else {
					...
					try {
						$PostLicence = file_get_contents("https://download.workcontrol.com.br?k={$PostData['licene_key']}&u={$PostData['user_email']}&p=" . hash('sha512',
								$PostData['user_password']) . "&v=" . ADMIN_VERSION . "&d=" . urlencode(BASE));
						$resultLicence = json_decode($PostLicence);

						if (!empty($resultLicence->trigger)):
							$jSON['trigger'] = AjaxErro("<b class='icon-warning'>ERROR:</b> {$resultLicence->trigger}",
								E_USER_ERROR);
						else:
							//CREATE LICENCE JSON
						...
						//var_dump($e);
						$jSON['trigger'] = AjaxErro("<b class='icon-warning'>ERRO:</b> Desculpe mas não foi possível comunicar com download.workcontro.com.br.<p>Favor tente mais tarde!</p>",
							E_USER_ERROR);
					}
