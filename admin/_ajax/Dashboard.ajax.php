    ...
    if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack) {
                ...
                if ($DateDiff >= 5) {
                    ...
                    try {
                        ...
                        if (!empty($resultLicence->trigger)) {
                            ...
                            //UPDATE LICENSE
                            $LicenseUpdate = str_replace('"license_auth_date":"' . $LicenseDomain->license_auth_date . '"',
                                '"license_auth_date":"' . date("Y-m-d H:i:s") . '"', $LicenseFile);
        ...
        switch ($Case) {
            ...
            case 'wc_login_fix':
                if (!empty($_SESSION['userLogin']) && $_SESSION['userLogin']['user_level'] >= 6) {
                    ...
                    if ($Read->getResult() && $Read->getResult()[0]['user_level'] >= 6) {
                        ...
                        $_SESSION['trigger_login'] = AjaxErro("<div class='al_center icon-warning'>Sua sessão expirou ou você não tem permissão para acessar o painel!</div>",
                            E_USER_ERROR);
            ...
            case 'onlinenow':
                ...
                $Read->ExeRead(DB_VIEWS_ONLINE, "WHERE online_endview >= NOW() {$Where} ORDER BY online_endview DESC",
                    "{$ParseString}");
                if (!$Read->getResult()) {
