		...
		switch ($Case):
			...
			case 'workcodes':
				if (empty($PostData['code_name']) || empty($PostData['code_script'])):
					$jSON['trigger'] = AjaxErro("<b>ERRO:</b> Para cadastrar um WC CODE é preciso informar pelo menos o título e o script. Favor tente novamente!",
						E_USER_ERROR);
				else:
					if (empty($PostData['code_id'])):
						...
						$jSON['trigger'] = AjaxErro("<b>ATUALIZADO COM SUCESSO:</b> O seu WC CODE foi atualizado com sucesso e você já pode ver a alteração em seu site!",
							E_USER_NOTICE);
					endif;
				endif;
				break;
			...
			case 'edit':
				...
				if (!$Read->getResult()):
					$jSON['trigger'] = AjaxErro("<b>ERRO AO OBTER WORK CONTROL CODE:</b> Você tentou editar um código que não existe ou foi removido!",
						E_USER_ERROR);
				else:
