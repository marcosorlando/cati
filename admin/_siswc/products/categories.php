...
if (DB_AUTO_TRASH):
    $Delete = new Delete;
    $Delete->ExeDelete(DB_PDT_CATS_TRAVI, "WHERE cat_title IS NULL AND cat_parent IS NULL AND cat_id >= :st", "st=1");
endif;
?>
...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()):
        ...
        foreach ($Read->getResult() as $Sector):
            $Read->FullRead("SELECT count(pdt_id) AS total FROM " . DB_PDT_TRAVI . " WHERE pdt_category = :sector", "sector={$Sector['cat_id']}");
            ...
            <header>
                ...
                <div>
                    ...
                    <span rel='product_category' callback='ProductsTravi' callback_action='cat_delete' class='j_delete_action_confirm btn btn_yellow icon-warning' style='display: none;' id='{$Sector['cat_id']}'>Deletar Categoria?</span>
                </div> 
               
            </header>";

            $Read->ExeRead(DB_PDT_CATS_TRAVI, "WHERE cat_parent = :sector", "sector={$Sector['cat_id']}");
            if ($Read->getResult()):
                foreach ($Read->getResult() as $Cat):
                    $Read->FullRead("SELECT count(pdt_id) AS total FROM " . DB_PDT_TRAVI . " WHERE pdt_subcategory = :cat", "cat={$Cat['cat_id']}");
                    ...
                    echo "<article class='product_subcategory' id='{$Cat['cat_id']}'>
                            ...
                            <div>
                                ...
                                <span rel='product_subcategory' callback='ProductsTravi' callback_action='cat_delete' class='j_delete_action_confirm btn btn_yellow icon-warning' style='display: none;' id='{$Cat['cat_id']}'>Deletar Categoria?</span>
                            </div>
                        </article>";
                endforeach;
            endif;
