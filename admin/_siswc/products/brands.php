<?php
    ...
    if (DB_AUTO_TRASH):
        $Delete = new Delete;
        $Delete->ExeDelete(DB_PDT_BRANDS_TRAVI, "WHERE brand_title IS NULL AND brand_id >= :st", "st=1");
    endif;
?>
...
<header class="dashboard_header">
    ...
    <div class="dashboard_header_search">
        <a title="Novo Fabricante" href="dashboard.php?wc=products/saleunit" class="btn btn_green icon-plus">Adicionar Fabricante!</a>
    </div>
</header>

<div class="dashboard_content">
    <?php
        $Read->ExeRead(DB_PDT_BRANDS_TRAVI, "ORDER By brand_title ASC");
        if (!$Read->getResult()):
            ...
            foreach ($Read->getResult() as $Brand):
                $Read->FullRead("SELECT count(pdt_id) as total FROM " . DB_PDT_TRAVI . " WHERE pdt_brand = :brand", "brand={$Brand['brand_id']}");
            ...
            <div class='box_content'>
                ...
                <span rel='product_brand' callback='ProductsTravi' callback_action='brand_remove' class='j_delete_action_confirm btn btn_yellow icon-warning' style='display: none;' id='{$Brand['brand_id']}'>Deletar Fabricante?</span>
            </div>
        </article>";
            endforeach;
        endif;
