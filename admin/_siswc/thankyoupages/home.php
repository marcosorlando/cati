<?php
    ...
    if (DB_AUTO_TRASH):
        ...
        //AUTO TRASH IMAGES
        $Read->FullRead("SELECT page_cover, page_logo FROM " . DB_THANKYOU_PAGES . " WHERE page_title IS NULL AND page_status = :st", "st=0");
...
<header class="dashboard_header">
  ...
  <div class="dashboard_header_search">
    <a title="Nova Página de Cliente" href="dashboard.php?wc=thankyoupages/create" class="btn btn_green icon-plus">Nova Thank You Page</a>
  </div>
...
<div class="dashboard_content">
    <?php
        ...
        $Read->ExeRead(DB_THANKYOU_PAGES, "ORDER BY page_title ASC, page_date DESC LIMIT :limit OFFSET :offset",
        ...
        if (!$Read->getResult()):
            ...
            foreach ($Read->getResult() as $PAGE):
                ...
                $page_cover = (!empty($page_cover) ? BASE . "/tim.php?src=uploads/thankyoupages/{$page_cover}&w=" . IMAGE_W / 2 . "&h=" . IMAGE_H / 2 . "" : "");
                $page_logo = (!empty($page_cover) ? BASE . "/tim.php?src=uploads/thankyoupages/{$page_logo}&w=" . IMAGE_W / 3 . "&h=" . IMAGE_H / 3 . "" : "");
