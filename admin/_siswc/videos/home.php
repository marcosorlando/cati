...
<div class="dashboard_content">
    ...
    $Read->ExeRead(DB_YOUTUBE, "WHERE video_start <= NOW() AND (video_end >= NOW() OR video_end IS NULL) ORDER BY video_date DESC LIMIT :limit OFFSET :offset", "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");

    if (!$Read->getResult()):
        $Pager->ReturnPage();
        Erro("<span class='al_center icon-notification'>Ainda não existem vídeos cadastrados em seu site. Comece cadastrando o primeiro!</span>", E_USER_NOTICE);
   else:
		?>
  <div class="testimony"style="display: none;">
