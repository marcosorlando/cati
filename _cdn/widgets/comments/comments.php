...
//COMMENT TYPE
    ...
    if ($CommentType == 'post'):
        ...
        echo "<header><h3>Olá{$UserName}, deixe seu comentário para <span class='text-small text-outside-line-full alt-font font-weight-600 text-uppercase text-extra-dark-gray'>{$CommentTitle}</span></h3></header>";
    ...
    elseif ($CommentType == 'product'):
        ...
        echo "<section class='comments' id='comments'>";
        echo "<header><h3>{$CommentTitle}</h3><h4>Confira opiniões e avaliações de clientes!</h4></header>";
    endif;

    if ($Read->getResult()):
        echo "<div class='comments_count'>Já temos {$CommentCount} comentário(s). <b>DEIXE O SEU </b><i class='fa fa-heart text-red'></i></div>";
        foreach ($Read->getResult() as $Comment):
            ...
            if (empty($LikeThisPost)):
                echo "<span class='like wc_like' id='{$Comment['id']}'><b>{$LikeCount}</b> <i class='text-blue fa fa-thumbs-up animated pulse infinite'></i> GOSTEI</span>";
            else:
                $LikeThisPost = null;
                echo "<span class='liked'><b>{$LikeCount}</b> <i class='fa fa-thumbs-up text-blue'></i> VOCÊ JÁ CURTIU ISSO!</span>";
            endif;
            echo "<span class='response wc_response' id='{$Comment['id']}' rel='{$UserComment['user_id']}'> <i class='fa fa-comments'></i> RESPONDER</span>";
            echo "</div>"; //Ui
            echo "<div class='comments_single_likes' id='{$Comment['id']}'><span><i class='text-red fa fa-heart animated pulse infinite'></i> {$Likes}</span></div>";
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $Response):
                    ...
                    if (empty($LikeThisPost)):
                        echo "<span class='like wc_like' id='{$Response['id']}'><b>{$LikeCount}</b> <i class='text-blue fa fa-thumbs-up animated pulse infinite'></i> GOSTEI</span>";
                    else:
                        $LikeThisPost = null;
                        echo "<span class='liked'><b>{$LikeCount}</b> <i class='text-blue fa fa-thumbs-up'></i> VOCÊ JÁ CURTIU ISSO!</span>";
                    endif;
                    echo "<span class='response wc_response' id='{$Response['id']}' rel='{$UserComment['user_id']}'><i class='fa fa-comments'></i> RESPONDER</span></span>";
                    echo "</div>"; //Ui
                    echo "<div class='comments_single_likes' id='{$Response['id']}'><span><i class='text-red fa fa-heart animated pulse infinite'></i> {$Likes}</span></div>";
